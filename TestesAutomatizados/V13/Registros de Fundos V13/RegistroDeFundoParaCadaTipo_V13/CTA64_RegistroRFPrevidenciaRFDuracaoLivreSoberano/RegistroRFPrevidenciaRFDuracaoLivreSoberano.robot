*** Settings ***
Documentation      Essa automação realiza um cadastro de um fundo do tipo RF Previdência RF Duração Livre Soberano
...
*** Test Cases ***
[Registro de Fundo]Registrar fundo do tipo RF Previdência RF Duração Livre Soberano
    Acessar o HUB    ${EMAIL}
    ...
    Preencher perfil da classe com argumentos RF previdencia V13    Renda fixa    Previdência    Previdência RF Duração Livre Soberano    DURACAO_LIVRE    SOBERANO
    ...
    Validar fundo em analise    PrevidenciaRFDuracaoLivreSoberano