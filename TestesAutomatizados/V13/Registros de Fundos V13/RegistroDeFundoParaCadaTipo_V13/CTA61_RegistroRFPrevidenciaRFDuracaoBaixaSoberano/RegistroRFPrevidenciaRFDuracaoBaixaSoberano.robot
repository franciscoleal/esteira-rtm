*** Settings ***
Documentation      Essa automação realiza um cadastro de um fundo do tipo RF Previdência RF Duração Baixa Soberano
...
*** Test Cases ***
[Registro de Fundo]Registrar fundo do tipo RF Previdência RF Duração Baixa Soberano
    Acessar o HUB    ${EMAIL}
    ...
    Preencher perfil da classe com argumentos RF previdencia V13    Renda fixa    Previdência    Previdência RF Duração Baixa Soberano    DURACAO_BAIXA    SOBERANO
    ...
    Validar fundo em analise    PrevidenciaRFDuracaoBaixaSoberano