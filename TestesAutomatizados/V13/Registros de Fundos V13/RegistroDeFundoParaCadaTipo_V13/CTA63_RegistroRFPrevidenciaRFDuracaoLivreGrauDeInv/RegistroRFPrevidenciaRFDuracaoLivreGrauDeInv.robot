*** Settings ***
Documentation      Essa automação realiza um cadastro de um fundo do tipo RF Previdência RF Duração Livre Grau de Inv.
...
*** Test Cases ***
[Registro de Fundo]Registrar fundo do tipo RF Previdência RF Duração Livre Grau de Inv
    Acessar o HUB    ${EMAIL}
    ...
    Preencher perfil da classe com argumentos RF previdencia V13    Renda fixa    Previdência    Previdência RF Duração Livre Grau Invest    DURACAO_LIVRE    GRAU_INVESTIMENTO
    ...
    Validar fundo em analise    PrevidenciaRFDuracaoLivreGrauDeInv