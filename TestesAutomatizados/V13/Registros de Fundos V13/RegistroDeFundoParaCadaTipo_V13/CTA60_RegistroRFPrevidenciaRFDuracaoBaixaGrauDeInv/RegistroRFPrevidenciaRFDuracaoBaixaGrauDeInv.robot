*** Settings ***
Documentation      Essa automação realiza um cadastro de um fundo do tipo RF Previdência RF Duração Baixa Grau de Inv
...
*** Test Cases ***
[Registro de Fundo]Registrar fundo do tipo RF Previdência RF Duração Baixa Grau de Inv
    Acessar o HUB    ${EMAIL}
    ...
    Preencher perfil da classe com argumentos RF previdencia V13    Renda fixa    Previdência    Previdência RF Duração Baixa Grau Invest    DURACAO_BAIXA    GRAU_INVESTIMENTO
    ...
    Validar fundo em analise    PrevidenciaRFDuracaoBaixaGrauDeInv