*** Settings ***
Documentation      Essa automação realiza um cadastro de um fundo do tipo RF Previdência RF Duração Média Soberano
...
*** Test Cases ***
[Registro de Fundo]Registrar fundo do tipo RF Previdência RF Duração Média Soberano
    Acessar o HUB    ${EMAIL}
    ...
    Preencher perfil da classe com argumentos RF previdencia V13    Renda fixa    Previdência    Previdência RF Duração Média Soberano    DURACAO_MEDIA    SOBERANO
    ...
    Validar fundo em analise    PrevidenciaRFDuracaoMediaSoberano