*** Settings ***
Documentation      Essa automação realiza um cadastro de um fundo do tipo RF Previdência RF Duração Alta Grau de Inv.
...
*** Test Cases ***
[Registro de Fundo]Registrar fundo do tipo RF Previdência RF Duração Alta Grau de Inv.
    Acessar o HUB    ${EMAIL}
    ...
    Preencher perfil da classe com argumentos RF Previdencia V13    Renda fixa    Previdência    Previdência RF Duração Alta Grau Invest    DURACAO_ALTA    GRAU_INVESTIMENTO
    ...
    Validar fundo em analise    PrevidenciaRFDuracaoAltaGrauDeInv